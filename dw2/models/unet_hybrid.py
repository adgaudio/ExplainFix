"""
Copy and then modify my unet.py implementation to support generalized haar

I create a new and mostly duplicate file here because I want to "archive"
results obtained with unet.py without risk of introducing bugs.
"""
import torch as T
...
class Block_DepthwiseSeparableConv2d(T.nn.Module):
    ...
    def __init__(self, channels_in:int, channels_mid:int, channels_out:int,
        ...
        convs = [
            ...
            batch_norms[0],
            activations[0],
            spatial_conv(channels_mid, channels_mid, kernel_size=3, groups=channels_mid, bias=False, **conv_kws[1]),
            batch_norms[1],
            activations[1],
...
class Downsample(T.nn.Sequential):
    def __init__(self, ch_in, ch_mid, ch_out, kernel_size, spatial_conv):
        super().__init__(
            Block_DepthwiseSeparableConv2d(
                ch_in, ch_mid, ch_out, kernel_size=kernel_size,
                paddings=(0,1,0), strides=(1,1,1), spatial_conv=spatial_conv),
            Block_DepthwiseSeparableConv2d(  # DOWNSAMPLE HERE
                ch_out, ch_mid, ch_out, kernel_size=kernel_size,
                paddings=(0,1,0), strides=(1,kernel_size,1), spatial_conv=spatial_conv)
        )
    ...
    def __init__(self, channels: Tuple[int]=(3,32,64,128,256,512,1024,),
        ...
        for i in range(len(channels)-1):
            kws = dict(
                ...
                ch_out=channels[i+1],
                kernel_size=3, spatial_conv=spatial_conv)
