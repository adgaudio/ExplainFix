...
class DCTConv2d(T.nn.Module):
    """Experimental idea.  I can't remember if/how it worked.

    Replace a spatial Conv2d layer with an explainable convolution structure
    based on the basis used for the Discrete Cosine Transform.  It wraps a 2d
    convolution with two 1x1 convolutions, and it inserts no non-linearities.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        """
        Inputs are all regular parameters of torch.nn.Conv2d, but additionally:
            :basis: which basis to use, one of {"DCT-II", "DCT-IV", "learned"}
        """
        super().__init__()
        self.kernel_size = _ensure_minlength(kernel_size, 2)
        basis_size = math.prod(self.kernel_size)
        # a hypothesis: each group of input channels represents a different input "image"
        # this works for depthwise convolution, but may fail miserably for normal "dense" convolution
        # if it's true for normal convolutions, it would give big compute savings.
        # maybe we'll need bias on conv_1x1 to work for the normal "dense" convolutions
        #  num_basis_images = lambda in_, out_, basis_size: int(in_/6*basis_size)
        mid_channels = num_basis_images * basis_size
        kws = dict(
            stride=stride, dilation=dilation)
        self.conv_1x1_in = T.nn.Conv2d(
            in_channels, mid_channels, kernel_size=1, bias=bias, **kws)
        self.conv_nxn_mid = T.nn.Conv2d(
            mid_channels, mid_channels, kernel_size=self.kernel_size,
            groups=mid_channels, bias=False,
            padding=padding, padding_mode=padding_mode, **kws)  # this bias is always false
        self.conv_1x1_out = T.nn.Conv2d(
            mid_channels, out_channels, kernel_size=1, bias=False, **kws)
        self._basis = basis
        self._num_basis_images = num_basis_images
        self.reset_parameters(spatial_only=True)

    def reset_parameters(self, basis:Union[str,T.Tensor]=None, spatial_only=False):
        # initialize 1x1 convolution params.
        if not spatial_only:
            self.conv_1x1_in.reset_parameters()
            self.conv_1x1_out.reset_parameters()
        # initialize spatial convolution parameters with a fixed basis
        if basis is None:
            basis = self._basis
        # --> first, consider if basis is to be learned, then do nothing special
        if basis == "learned":
            self.conv_nxn_mid.reset_parameters()
            self.conv_nxn_mid.requires_grad_(True)
            return
        # --> initialize weights with a basis
        self.conv_nxn_mid.requires_grad_(False)
        device = self.conv_nxn_mid.weight.device
        N, M = self.kernel_size
        NM = N*M
        if isinstance(basis, str):
            basis = dct_basis_2d(*self.kernel_size, basis)
            assert basis.shape == (NM, *self.kernel_size), 'code error: basis vecs expected on rows'
            basis = T.tensor(basis, dtype=T.float, device=device)
        else:
            basis = basis.float().to(device)
        assert basis.shape == (NM, N, M), 'sanity check'
        try:
            self.conv_nxn_mid.weight.data[:] = basis.reshape(NM, 1, N, M).tile(self._num_basis_images,1,1,1)
        except:
            import IPython ; IPython.embed() ; import sys ; sys.exit()
        assert self.conv_nxn_mid.bias is None, 'sanity check'

    def forward(self, x):
        x = self.conv_1x1_in(x)
        x = self.conv_nxn_mid(x)
